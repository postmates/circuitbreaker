...
# circuit breaker defined in a separate file
# p("program starting")
...
@CircuitBreaker(failure_threshold=8, name="rogan-tset", expected_exception=ValueError, recovery_timeout=4)
def test():
    raise ValueError("sorry rogan")
...
if os.fork():
    prefix = "parent:{}".format(str(os.getpid()))
    # b = B()
    for i in xrange(10):
        try:
            test()
    ...
    prefix = "child:{}".format(str(os.getpid()))
    for i in xrange(10):
        try:
            test()
        except Exception as e:
